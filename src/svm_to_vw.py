...
FEATURE_FIELD = {'feature3': [20113, 39, 2, 7, 3554, 30, 4],
                 ...
                 'feature9': [20113, 39, 10, 3554, 6, 7, 10, 30, 4, 7, 12, 2,
                              86, 86],
                 'feature10': [55907, 39, 9, 5268, 1, 6, 7, 11, 30, 4, 7, 8, 2,
                               87, 87]}


def svm_to_vw(svm_file, vw_file, feature_name):

    feature_per_field = FEATURE_FIELD[feature_name]
    max_idx_per_field = np.cumsum(feature_per_field)
    n_field = len(feature_per_field)

    with open(vw_file, 'w') as fout, open(svm_file) as fin:
        ...
        for i, row in enumerate(fin, 1):
            ...
            row = row.strip().split(' ')
            new_row = [row[0] if row[0] == '1' else '-1']
            features = row[1:]

            i_field = 0
            prev_max_idx = 0
            new_row.append('|{:c}'.format(i_field + ord('a')))
            for feature in features:
                ...
                new_row.append('{}:{}'.format(idx - prev_max_idx, value))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vw-file', '-v', required=True, dest='vw_file')
    ...
    start = time.time()
    svm_to_vw(args.svm_file, args.vw_file, args.feature_name)
