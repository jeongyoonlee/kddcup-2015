...
def train_predict(train_file, test_file, predict_valid_file, predict_test_file,
                  n_est, depth, n_fold=5, n_bag=50):

    feature_name = os.path.basename(train_file)[:-4]
    logging.basicConfig(format='%(asctime)s   %(levelname)s   %(message)s',
                        level=logging.DEBUG,
                        filename='et_bag{}_{}_{}_{}.log'.format(n_bag,
                                                                n_est,
                                                              depth,
                                                              feature_name))
    ...
    et = ET(n_estimators=n_est, max_depth=depth, random_state=2015,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n-bag', default=100, type=int, dest='n_bag')
